...
if ( ! function_exists( 'assignment_theme_posted_on' ) ) :
	...
	function assignment_theme_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'assignment-theme' ),
	...
	function assignment_theme_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'assignment-theme' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'assignment-theme' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'assignment-theme' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'assignment-theme' ),
					array(
						'span' => array(
							'class' => array(),
						),
